...
\subsubsection{Summary \& Limitations}
...
\subitem{Evaluation of Model-Generated Trajectories.}
...
% One common approach is to first pretrain a Video Prediction Model $p(o_{t+1} \mid o_t)$ on $D_{video}$ to learn the dynamics and physics of the world and then transfer this knowledge to a robot dynmaics model $p(o_{t+1} \mid o_t, a_t)$ using $D_{robot}$, which additionally incorporates the influence of robot actions on future states of the environment.
% Anothern approach is pretraining an alternative-action video predictor $p_{alt}(o_{t+1} \mido_t, \hat{a}_t)$ with an alternative action representation $\hat{a}_t$ extracted from video data, which can then be used to condition a downstream action decoder trained on robot data. Conditioning on an $\hat{A} $ allows for more control over video generations, and often be a mapping can be obtained from $\hat{A} $ to the robot action-space.
% Anothern approach is to jointly pretrain a dynmaics model $p(o_{t+1} \mid o_t, a_t)$ on $D_{video}$ and $D_{robot}$, where the video data provides rich information about the environment's dynamics, while the robot data grounds the model in actionable interactions. UniSim can handle temporally extended actions in various modalities such as motor controls, language descriptions, and actions extracted from camera motions and
% other sources.
