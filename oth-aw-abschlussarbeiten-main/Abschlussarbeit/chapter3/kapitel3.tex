...
% In addition, general-purpose robots could play an important role 
In household settings, they could assist with everyday tasks such as tidy up rooms, loading and unloading dishwashers, setting tables, or acting as general assistants for example helping to carry objects \cite{Yang2025PreservingSO}.
...
\section{Strategies to Overcome the Bottleneck}
...
\subsection{Data Categories in Robotics}
Before comming to approaches for addressing the data bottleneck in robotics, it is important to clarify which data sources are relevant for robot learning and how such data can be acquired.
...
\paragraph{Simulated Data}
	...
	\begin{itemize}
		...
		are used for manipulation and navigation tasks and includes motion planning algorithms such as A*, PRM, or RRT* to compute collision-free trajectories. Frameworks such as MoveIt \cite{MoveIt2Documentation} integrate these planning methods with low-level robot controllers and are widely used in robotics applications \cite{DBLP_journals_corr_abs_2103_12826}. These approaches can generate high-quality demonstrations but often require accurate environment models and can become computationally expensive in complex scenarios.

		\item \textbf{Obtaining Policies via Reinforcement Learning}
		means, training a \ac{RL} policy, froze it and use it to generate expert demonstration trajectories for downstream \ac{IL} \cite{ParkerHolder2022EvolvingCW, Liang2024EurekaverseEC}. Compared to hand-crafted or planning-based approaches, \ac{RL} policies can better adapt to high-dimensional and complex tasks, although training them typically requires large amounts of interaction data and computational resources \cite{McCarthy_2025}.
		In simulation, the learning process can be simplified by leveraging privileged information available there and by training specialized expert policies for individual subtasks \cite{Ha2023ScalingUA, Geng2023UniDexGraspID, Dalal2023ImitatingTA}. These policies can subsequently be coordinated to solve complex tasks.
...
In modern robot learning systems, simulation data, real-world data and internet data are commonly combined, as each category provides complementary strengths and addresses different limitations. 
...
\subsection{Strategies to Reduce the Amount of Robot Data}
...
The x-axis represents the range of behaviors expected from a generalist robot, while the y-axis indicates the level of information contained in the data. The figure shows that internet data covers a broader range of behaviors than robot datasets but lacks essential low-level information required for robotics. Bridging this gap is a key challenge in Learning from Videos (LfV).

\begin{figure}[!ht]
	\centering
	\includegraphics[width=1.0\linewidth]{./chapter4/media/DataBottleneck}
	\caption[Problem: Data Bottleneck in Robotics]{Problem: Data Bottleneck in Robotics. The x-axis represents the spectrum of behaviors that a generalist robot is expected to perform, while the y-axis reflects the different information levels present in the data sources. Image source: \cite{McCarthy_2025}.}
	\label{fig:DataBottleneck}
\end{figure}
% Internet data covers a wider range of behaviors than robot datasets but lacks low-level robotic information required for control, making this gap a key challenge in Learning from Videos (LfV).
...
Ideally, for every relevant behavior, both low-level robot data and internet data would be available, allowing a model to establish connections across different information levels. However, there is a significant lack of low-level robot data for many behaviors that are well represented in internet data (see ``Missing Info'' area). 
...
This absence of fine-grained information poses a significant challenge for generalizing robot learning beyond the limitations of available robot datasets \cite{McCarthy_2025}. Overcoming this gap and effectively leveraging both broad internet data and precise robot data is a central objective in \ac{LfV} approaches \cite{McCarthy_2025}.
\ac{LfV} aims to exploit this abundance of video data to improve robotic learning compared to learning from robot data alone. 
% \cite{McCarthy_2025} says, the goal of \ac{LfV} is to leverage our combined \ac{LfV} dataset, $D = \{D_{robot}, D_{video}\}$, to obtain an improved policy $\pi(a_t | s_t)$ versus when learning from $D_{robot}$ alone. 
% \paragraph{Formalism} 
The data in \autoref{fig:DataBottleneck} can be formalized as follows: $D$ refers to the complete training dataset used for learning the robotic policy, which can be divided into two subsets: $D = \{D_{robot}, D_{video}\}$. 
$D_{robot}$ contains robot interaction data collected from physical robots in real-world or simulation environments, including observations and the complete robot trajectory. Each sample in $D_{robot}$ consists of transition tuples of the form $(o_t, a_t, r_t, o_{t+1})$, 
% where $o_t$ is the robot observation, which can contain other information like depth or teactile data, $a_t$ is the action taken by the robot, $r_t$ the received reward after taking the action (which may be missing) and $o_{t+1}$ the next observation.
$D_{robot}$ contains video (demonstration) data and can be obtained from external sources such as internet videos. A video clip is represented as $\tau=(o_0, o_1, ..., o_T)$, where $\tau$ denotes the full video clip and each observation $o_t$ corresponds to an RGB image, which may additionally be paired with language descriptions, action annotations, or other auxiliary modalities.
% \begin{itemize}
% 	\item $D$: Complete training dataset used for learning the robotic policy, which can be divided into two subsets: $D = \{D_{robot}, D_{video}\}$. 
% 	% $D$ can include real-world data, simulated data and internet data.

% 	\item $D_{robot}$: Contains robot interaction data collected from physical robots in real-world or simulation environments, including observations and the complete robot trajectory. Each sample in $D_{robot}$ consists of transition tuples of the form $(o_t, a_t, r_t, o_{t+1})$, where $o_t$ is the robot observation, which can contain other information like depth or teactile data, $a_t$ is the action taken by the robot, $r_t$ the received reward after taking the action (which may be missing) and $o_{t+1}$ the next observation.

% 	\item $D_{video}$: Contains video (demonstration) data and can be obtained from external sources such as internet videos. A video clip is represented as $\tau=(o_0, o_1, ..., o_T)$, where $\tau$ denotes the full video clip and each observation $o_t$ corresponds to an RGB image. Depending on the dataset, videos may additionally be paired with language descriptions, action annotations, or other auxiliary modalities.
% \end{itemize}
...
By Combining $D_{\text{robot}}$ and $D_{\text{video}}$, several improvements can be expected, instead of using only $D_{\text{robot}}$. Robot datasets primarily provides information about low-level skills, such as grasping or locomotion \cite{Chen2024RH20TPAP}, internet-scale videos contains a much broader variety of tasks and human behaviors. By linking robotic actions to higher-level task structures observed in videos, \ac{LfV} systems may generalize to tasks that are not explicitly represented in $D_{\text{robot}}$ \cite{Eze2024LearningBW, Brohan2023RT2VM, Du2023LearningUP, Wu2023UnleashingLV, Wang2023MimicPlayLI}.
By combining robot data and video data with language annotations, models may learn to connect low-level robotic skills with rich semantic abstractions and higher-level world knowledge obtained from textual data [50], which may enable improved task understanding, generalization and and the ability to solve previously unseen tasks [47, 50, 119]. LLMs and vision models have already demonstrated that training on large amounts of data has led to the emergence of capabilities [47, 119]
Video data may also improve performance on tasks that are already represented within the robot dataset $D_{\text{robot}}$. 
% \begin{itemize}
% 	\item \textbf{Generalization beyond $D_{\text{robot}}$:}
% 	By linking robotic actions to higher-level task structures observed in videos, \ac{LfV} systems may generalize to tasks that are not explicitly represented in $D_{\text{robot}}$ \cite{Eze2024LearningBW, Brohan2023RT2VM, Du2023LearningUP, Wu2023UnleashingLV, Wang2023MimicPlayLI}.
...
% 	% Similar to advances in language and vision models, where training on large-scale internet data have led to the emergence of capabilities that were not explicitly trained for \cite{Radford2019LanguageMA, Brown2020LanguageMA}, we can expect similar progress in robotics. Large-scale videos with language annotations can lead to learning connections between low-level skills and rich semantic abstractions or higher-level world knowledge \cite{Achiam2023GPT4TR}.
% 	%  or higher-level world knowledge \cite{Achiam2023GPT4TR}. 
...
% 	\item \textbf{Improvements in-distribution of $D_{\text{robot}}$:}
% 	Video data may also improve performance on tasks that are already represented within the robot dataset $D_{\text{robot}}$. 
% 	% Robot demonstrations are expensive to collect and additional video data can lead to more robust visual and task representations to improve data efficiency and task success \cite{Nair2022R3MAU, Wu2023UnleashingLV}.
...
\textit{Low-Level Robot Information} 
refers to sensory and control signals that directly describe the robot's physical state, motion and interaction with its environment. This includes, for instance, end-effector pose, joint positions, joint velocities, joint torques and gripper state (e.g., open or closed). Additionally, low-level information includes information about the physical forces acting on the robot itself. In simulation environments, they may include physical details of objects in the environment, such as contact forces, object properties or object dynamics.
...
Proprioceptive information refers to the robot's internal sense of its own body and movement like the robot's end-effector pose which implicity inlcudes joint angles, joint velocities and gripper configuration. It allows the robot to understand its current state and adjust its actions accordingly to reach the task goal.
% This information is crucial for executing accurate and stable motions, as it allows the robot to understand its current state and adjust its actions accordingly. 
	...
	% \end{itemize}
Tactile data refers to information obtained through physical contact between the robots sensors and its environment.
%  aquired by the robots sensors. 
It is comparable to the human sense of touch and include contact forces, pressure distributions, slip detection, surface texture information and vibration signals. This feedback is critical for maintaining stable and safe object manipulation. For example, tactile sensors can indicate whether an object has been successfully grasped 
% , which are aquired using the robot sensors.
%  These measurements are typically acquired using sensors placed on robot grippers, fingertips, or other contact surfaces.
% For instance, during grasping, tactile sensors can indicate whether an object has been successfully grasped.
% , whether it is slipping, or whether excessive force is being applied. 
% This feedback is critical for maintaining stable and safe object manipulation.
...
\textit{Mid-Level Video Information}
refers to visual observations that capture the dynamics of objects, agents and environments over time. In contrast to robot data, video data does not include direct information about actions or internal robot states, instead it provides sequences of RGB observations that encode how the visual scene evolves over time.
It includes visual dynamics such as object motion, changes in spatial configurations and interactions between entities in the scene. For example, videos can capture how objects are moved, grasped, or manipulated by humans, as well as how these actions affect the surrounding environment. Although no explicit force or control signals are available, the physical behavior of objects (e.g., sliding, rotating, or deforming) can often be inferred implicitly from the visual changes across frames. 
A key aspect of mid-level video information is that it encodes \textbf{physical behavior at a perceptual level}. While it does not provide direct measurements of forces or torques, it still reflects underlying physical principles such as gravity, friction and object permanence through observable motion patterns. 
For instance, a video of a person pouring water into a glass implicitly contains information about liquid flow dynamics and object interaction constraints.
In addition, many video datasets are paired with textual annotations, captions, or narration, which link visual observations to semantic concepts. This allows models to associate language with observed actions and events, such as "opening a door" or "placing an object on a table". 
This helps bridge the gap between perception and high-level task understanding, enabling learning of more abstract representations that go beyond purely visual dynamics.
% Through such alignment, video data helps bridge the gap between perception and high-level task understanding, enabling learning of more abstract representations that go beyond purely visual dynamics
...
\textit{High-Level Language Information}
refers to abstract semantic descriptions of tasks, concepts and behaviors expressed in natural language. In contrast to visual or robot-centric signals, language does not encode direct sensory observations or motor commands but provides structured knowledge about goals, constraints and task structures. This includes descriptions of concepts, relational knowledge and symbolic abstractions of actions and environments.
Such information is particularly important for representing task intent and enabling generalization across different embodiments and environments. Language can describe tasks at multiple levels of abstraction, ranging from low-level action descriptions, like ``pick up the cup'' to high-level procedural or goal-oriented instructions, like ``clean the table'', which can be decomposed into intermediate steps such as removing objects, wiping the surface and reorganizing items. Such decompositions reflect implicit reasoning about task structure and enable more systematic policy learning. Therefore, language plays a central role in \textbf{planning and reasoning} and provides a medium for expressing sequential structure, subgoals and dependencies between actions.
% Therefore ta, which can be decomposed into intermediate steps such as removing objects, wiping the surface and reorganizing items). Such decompositions reflect implicit reasoning about task structure and enable more systematic policy learning.
% % This hierarchical structure allows models to connect concrete behaviors with more abstract task specifications.
...
Furthermore, language serves as a bridge for incorporating prior knowledge about object affordances, pysical interactions or human behaviors into robotic systems for example from \acp{LLM}. 
% Through textual descriptions, models can access commonsense knowledge about object affordances, physical interactions and typical human behaviors. 
This enables improved generalization to unseen tasks by leveraging semantic relationships that are not explicitly present in robot or video data alone.
...
% Although internet-scale video data offers significant potential for robotic learning, it also introduces several fundamental challenges, like missing action labels, missing low-level information, distribution shift, high-dimensionality and noise, as well as issues related to controllability, stochasticity and partial observability.
In contrast to robot interaction datasets, videos are typically high-dimensional, noisy, weakly labeled and highly diverse. Extracting meaningful representations and task-relevant information from such data is therefore a non-trivial problem \cite{McCarthy_2025}. Although internet-scale video data offers significant potential for robotic learning, it also introduces fundamental challenges, such as missing action labels and low level information, distribution shift, as well as issues related to the high-dimensionality and noise nature or the stochasticity and partial observability of videos.
...
\begin{enumerate}
	...
	\item \textbf{High-dimensionality, Noise and Redundancy:}
	...
	\item \textbf{Controllability, Stochasticity and Partial Observability:}
