...
\texttt{Goal of Learning from Video (LfV).} \cite{McCarthy_2025} says, the goal of LfV is to leverage our combined LfV dataset, $D = {D_{robot}, D_{video}}$, to obtain an improved policy $\pi(a_t | s_t)$ versus when learning from $D_{robot}$ alone.



LfV aims to exploit this abundance of video data to improve robotic learning. 
...
Roboter daten enthalten eher low-level und mid-level informationen. Low level Informationen sind auf den Roboter bezogen und enthalten information about the robot's actions, kräfte die auf den Roboter selbst wirken, bzw. in simulierten Umgebungen auftreten / in Simulationen können pysikalische daten von anderen sich in der Umgebung befindenden Objekten verfügbar sein, desweiteren . 
...
\texttt{Mid-level video information.}
...
\texttt{High-level language information.}
