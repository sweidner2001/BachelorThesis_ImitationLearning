...
% Classical robotics techniques are insufficient as they cannot handle unseen and unstructured scenarios \cite{Krotkov_DARPA _2017}.
...
Recent work suggests that similar scaling behavior may also apply to robotics and imitation learning. For example, studies show that robots generalize better when they are trained on more diverse environments and objects \cite{lin2024data}. Moreover, approaches such as large-scale behavioral cloning across multiple tasks have shown promising zero-shot generalization capabilities in novel settings \cite{rt12022arxiv}.
...
However, unlike vision or language domains, robotics is fundamentally constrained by the cost and complexity of data collection. High-quality demonstrations require physical real world interaction, are time-consuming to acquire, and are often limited in diversity due to hardware and safety constraints \cite{tsujiSurveyImitationLearning2026}. As a result, available datasets are usually smaller and less diverse. 
This data scarcity remains a major bottleneck for achieving similar levels of scalability and generalization in robot learning and is the main challenges for developing general-purpose robotic systems \cite{tsujiSurveyImitationLearning2026}.
% This lack of large-scale data remains one of the main challenges for developing general-purpose robotic systems.


In contrast, robotics faces a fundamental limitation: the lack of large, diverse, and high-quality datasets. This data scarcity remains a major bottleneck for achieving similar levels of scalability and generalization in robot learning.

Recent work indicates that similar scaling behavior may also hold in robotics and imitation learning. For instance, studies on robotic manipulation demonstrate that policy generalization improves with the diversity of environments and objects, often exhibiting power-law relationships with respect to dataset scale . Moreover, approaches such as large-scale behavioral cloning across multiple tasks have shown promising zero-shot generalization capabilities in novel settings \cite{jang2022bcz}. 

However, unlike vision or language domains, robotics is fundamentally constrained by the cost and complexity of data collection. High-quality demonstrations require physical real world interaction, are time-consuming to acquire, and are often limited in diversity due to hardware and safety constraints.

This results in a significant data bottleneck, which remains one of the primary obstacles to scaling imitation learning methods toward general-purpose robotic systems .
...
\begin{figure}[!ht]
	\centering
	\includegraphics[width=0.6\linewidth]{./chapter3/media/DataBottleneck}
