...
How can we address this data bottleneck in robotics? 

Before discussing specific approaches, it is important to clarify which data sources are relevant for robot learning and how such data can be acquired.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Hand-crafted controllers are based on classical control methods such as PD/PID controllers, Linear Quadratic Regulators (LQR), or Model Predictive Control (MPC). These methods are particularly effective for low-dimensional tasks, including balancing, set-point tracking, or waypoint following, where they can generate stable and accurate trajectories. However, they often struggle in complex or high-dimensional environments.
		\item \textbf{rule-based and planning-based systems:} 
		For manipulation and navigation tasks, motion planning algorithms such as A*, PRM, or RRT* can be used to compute collision-free trajectories. Frameworks such as [MoveIt](https://moveit.ai/?utm_source=chatgpt.com) integrate these planning methods with low-level robot controllers and are widely used in robotics applications. These approaches can generate high-quality demonstrations but often require accurate environment models and can become computationally expensive in complex scenarios.
...
In der Praxis werden meist alle complementary sources of data gemeinsam genutzt. 



Zusammenspiel der Datenquellen
Realworld daten Daten sind besonders wertvoll für die Feinabstimmung von Modellen, die in Simulationen vortrainiert wurden, um die Diskrepanz zwischen Simulation und Realität zu überbrücken.
