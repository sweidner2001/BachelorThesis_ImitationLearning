...
\begin{figure}
	\centering
	\includegraphics[width=1.0\linewidth]{./chapter2/media/TransformerArchitectur3}
...
\begin{figure}
	\centering
	\includegraphics[width=1.0\linewidth]{./chapter2/media/AttentionWords2}
...
\textit{Compute Attention Weights.} In the self-attention mechanism, each token embedding vector $x^{(i)}$ is transformed into three different representations: query $q^{(i)}$, key $k^{(i)}$ and value $v^{(i)}$. These representations are obtained through learnable linear transformations with parameters ($W_q$, $W_k$, $W_v$) that are optimized during training.
\begin{itemize}
	\item \textbf{Query $q^{(i)}=x^{(i)}W_q$:} The query vector represents the token for which we want to compute attention. It is used to determine how much attention this token should pay to other tokens in the sequence.
	\item \textbf{Key $k^{(i)}=x^{(i)}W_k$:} The key vector represents the token against which the query is compared. It is used to compute the relevance score between the query and the token.
	\item \textbf{Value $v^{(i)}=x^{(i)}W_v$:} The value vector contains the actual information of the token. It is weighted by the attention scores to produce the final output of the self-attention mechanism.
\end{itemize}

\textit{Self-Attention.} The query vectors are then compared with the key vectors of all tokens in the sequence using the dot product (scalar product), producing relevance scores that indicate how strongly tokens are related to one another. To stabilize training, these scores are scaled by the square root of the key dimension $d_k$ and normalized using a softmax function, resulting in attention weights. Finally, the attention weights are applied to the corresponding value vectors, and the weighted values are aggregated to produce the new context vector $z^{(i)}$ and the output of the self-attention layer, see \autoref{fig:TransformerAttentionKeyQueryValueSimply}. 
\begin{equation}
	z^{(i)} = \text{softmax}\left(\frac{q^{(i)} \cdot k^{(i)T}}{\sqrt{d_k}}\right)v^{(i)}
\end{equation}
...
Instead of computing attention separately for each token, the Transformer performs the computation for the entire sequence using matrix operations, see \autoref{fig:TransformerSingleHeadAttention}. The matrix product $QK^T$ computes the pairwise similarity scores between all tokens in the sequence in parallel, resulting in the attention score matrix.
\begin{equation}
	\text Z = {Attention}(Q,K,V)=\text{softmax}\left( \frac{QK^T}{\sqrt{d_k}} \right)V
\end{equation}
...
The number of heads is chosen such that the input embedding dimension is split across the heads. For example, if the input embedding dimension is 768 and there are 12 heads, each head would operate on a subspace of 64 dimensions. Otherwise, the output dimensions will not match the input dimensions.
...
For example, in computer vision, the Vision Transformer (ViT) applies the same attention mechanism to image patches instead of words, allowing it to capture spatial relationships in images effectively.
In a similar way, in computer vision, the attention mechanism can help a model focus on relevant parts of an image, such as identifying objects or understanding spatial relationships between different regions of the image.
