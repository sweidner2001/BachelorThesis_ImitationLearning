...
An Artificial Neural Network (ANN) is a machine learning model inspired by the structure of the biological brain. It consists of interconnected computational units called neurons, which process information and learn complex relationships from data \cite{Goodfellow2016DeepLearning, Bishop2006PatternRecognitionML}. 

The fundamental building block of a neural network is the artificial neuron. A neuron receives one or more input values, computes a weighted sum of these inputs, adds a bias term, and applies a nonlinear activation function to produce an output. By combining many neurons into layers and connecting these layers together, neural networks can learn increasingly complex representations of the input data \cite{Goodfellow2016DeepLearning, Bishop2006PatternRecognitionML}.


An Artificial Neural Network (ANN) is a machine learning model inspired by the structure of the biological brain and designed to learn complex relationships from data \cite{Goodfellow2016DeepLearning, Bishop2006PatternRecognitionML}. Neural networks consist of interconnected computational units called neurons, which are organized into layers and trained to perform tasks such as classification, regression, and control.

Unlike traditional algorithms that rely on manually fixed designed rules or features, neural networks automatically learn relevant patterns directly from data. Each neuron receives input values, computes a weighted sum, adds a bias term, and applies a nonlinear activation function to produce an output. By combining many neurons across multiple layers, neural networks can learn hierarchical representations and approximate highly complex functions \cite{Goodfellow2016DeepLearning, Bishop2006PatternRecognitionML}.
...
\begin{equation}
    a = \phi(z) = \phi\left(\sum_{i=1}^{n} w_i x_i + b\right).
...
\begin{equation}
    a = \phi\!\left(\mathbf{w}^T \mathbf{x} + b\right).
...
The output 
z
z is a linear combination of the inputs. To enable learning of non-linear patterns, a non-linear activation function
f(⋅) is applied, giving the final output a=f(z). Without such a non-linearity, stacking multiple neurons would still only yield a linear model. Common activation functions today include ReLU, sigmoid, and tanh.
...
The resulting value is then transformed by a nonlinear activation function $f(\cdot)$
...
where $y$ denotes the output of the neuron.

The activation function introduces nonlinearity into the model, enabling neural networks to learn complex relationships between inputs and outputs. Common activation functions include the sigmoid function, the hyperbolic tangent (tanh), and the Rectified Linear Unit (ReLU). Without a nonlinear activation function, a network consisting of multiple layers would collapse into a single linear transformation and would therefore be unable to represent nonlinear functions.




\textit{Mathematical Formulation.}
A perceptron receives multiple inputs $(x_1, x_2, \ldots, x_n)$, each associated with a trainable weight $(w_1, w_2, \ldots, w_n)$. The weighted inputs are summed together with an optional bias term $(b)$, see \autoref{eq:perceptron}. 
\begin{equation}
...
\begin{figure}[!t]
	\centering
	\includegraphics[width=0.7\linewidth]{./chapter2/media/TransformerArchitectur3}
...
After the self-attention computation, the resulting representations are passed through a layer normalization step, followed by a feed-forward network (MLP). A second layer normalization is applied after this transformation. This sequence of operations forms a single Transformer block, which can be stacked multiple times to increase model capacity \cite{Vaswani2017AttentionIA}.
...
\textit{Masked Multi-Head Attention.} Masked multi-head self-attention operates on the decoder's own input sequence, but with a causal mask that prevents each position from attending to any future positions \cite{Raschka2024BuildLLM}. The causal mask ensures that future tokens receive an attention weight of zero, making them invisible to the current position and ensures that the prediction for the next token depends only on previously generated tokens. 
...
\textit{Cross-Attention.} In cross-attention, the decoder's hidden states from the masked attention layer serve as queries $Q$, while the encoder's output serve as keys $K$ and values $V$, allowing the decoder to condition its predictions on the encoded source sequence \cite{Vaswani2017AttentionIA}.
...
\section{Foundation Model \& Multi-Model Model}
...
\begin{figure}[!ht]
	\centering
	\includegraphics[width=0.7\linewidth]{./chapter2/media/FoundationModel_vs_MultiModalModel.png}
