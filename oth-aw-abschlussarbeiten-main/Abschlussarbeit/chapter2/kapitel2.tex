...
A Transformer is a deep neural network architecture designed to process sequential data, especially text. It's the foundation for most
modern LLMs, enabling them to process and understand language
effectively.
...
% er die nächste Ausgabeeinheit produziert. Die Verfahrensweise kennen wir.
% Im Unterschied zu den Sequence-to-Sequence-Modellen aus dem vorherigen Kapitel verwendet der Transformer allerdings weder eine rekurrente noch eine konvolutionale Schicht
% zur Produktion des Kontextvektors oder der finalen Ausgabe. Stattdessen setzt er auf ein
% Verfahren, das sich Self-Attention nennt.
...
% - GPT models utilize self-supervised learning, where the model learns from
% the data itself without requiring explicit labels. In the case of GPT, the
% next word in a sentence serves as the label for the model to predict,
% allowing for training on massive unlabeled text datasets.
% - The transformer architecture utilizes an attention mechanism that allows
% the LLM to selectively access the entire input sequence when generating
% output, word by word
