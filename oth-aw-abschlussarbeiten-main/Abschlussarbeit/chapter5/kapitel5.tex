...
\begin{itemize}
    ...
    \item Later, a \textbf{Decoder} or \textbf{Alignment Model $a_t = f_{align}(\hat{a}_t)$} maps these latent actions to the robot's real motor commands.    
\end{itemize}
...
Once $\hat{A}$ is defined, the same representation can be used for:
\begin{itemize}
    \item policies $\pi_{\hat{A}}(\hat{a}_t \mid o_t)$,
    \item dynamics models $p(o_{t+1} \mid o_t, \hat{a}_t)$ or
    \item state-action value functions $Q(o_t, \hat{a}_t)$,
\end{itemize}
