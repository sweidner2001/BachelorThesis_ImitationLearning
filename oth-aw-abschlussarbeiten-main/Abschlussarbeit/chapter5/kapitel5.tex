...
\paragraph{General Overview of Video Representation Learning.}
...
Video prediction models (VPMs) aim to model the conditional distribution over future visual observations given past context \cite{McCarthy_2025}. The standard formulation is next-frame prediction
\begin{equation}
...
Trained on large-scale video data, these models implicitly learn priors over physical dynamics, object interactions, and human behavior, making them relevant for robotic learning and imitation learning \cite{McCarthy_2025}.
...
\begin{itemize}
    ...
    \item \texttt{Representation Learning:} Learned latent spaces capture semantically meaningful information about objects, motion, and interactions, which can be transferred to downstream imitation learning and reinforcement learning tasks \cite{Wu2023UnleashingLV, McCarthy_2025}.
...
VPMs can be conditioned on Alternative Actions \cite{Bruce2024GenieGI}, language instructions \cite{Yang2024VideoAT, BarTal2024LumiereAS, Kondratyuk2023VideoPoetAL}, or goal images \cite{Hoppe2022DiffusionMF}, enabling structured control over generated futures \cite{McCarthy_2025, Yang2024VideoAT}.
...
\begin{itemize}
    ...
    Large language models (LLMs) have demonstrated strong capabilities as high-level planners in robotics \cite{Ahn2022DoAI}. However, their lack of direct grounding in physical perception limits their reliability in real-world environments.
    ...
    \begin{itemize}
        \item improved grounding of symbolic reasoning in observed physical states %ToDo Source
        \item better modeling of object interactions and environment dynamics %ToDo Source
        \item more consistent closed-loop reasoning, where predictions are continuously informed by perceptual feedback \cite{McCarthy_2025}.
    \end{itemize}
...
Existing approaches can be broadly categorized into compositional pipelines, LLM-based architectures with adaptors, and natively multi-modal sequence models \cite{McCarthy_2025}.
...
\begin{itemize}
    ...
    For example, an Image-Language Model is first used to process individual video frames by answering frame-level questions. The resulting intermediate descriptions are then aggregated using a Large Language Model (LLM), which generates a global video summary \cite{Chen2023VideoCT}. While effective in practice, these approaches do not involve end-to-end video training, and therefore may fail to learn rich temporal video representations \cite{McCarthy_2025}.

    \item \textbf{Leveraging Pretrained LLMs via Adaptors and Finetuning:}
    A common strategy is to combine a pretrained video encoder with a pretrained LLM \cite{Lin2023VideoLLaVALU, Maaz2023VideoChatGPTTD, Papalampidi2023ASR, Zhang2023VideoLLaMAAI}. Since LLMs operate on token sequences, an adaptor module is introduced to project video features into the LLM input space. With the adaptor, the video encoder and LLM can be trained end-to-end on video-text data, allowing the model to learn to extract video representations that are aligned with language generation.
    ...
    The model is typically trained autoregressively using next-token prediction over this joint sequence, i.e.,
    \begin{equation}
    p(x_t \mid x_{<t}),
    \end{equation}
...
\paragraph{Next-observations-as-Actions:}
