...
\label{chap:LearningFromVideosDataModalityAndChallenges}
\minitoc

Videos are a challenging data modality, as you have seen in Section~\ref{sec:ChallengesOfVideoData}. This chapter addresses the two key questions for \ac{LfV} research:
\begin{enumerate}
...
\end{enumerate}
First different approaches are reviewed for extracting foundational knowledge from internet video, including video encoders, video prediction models and video-to-text models. Then methods for transferring video-extracted knowledge to robotics are discussed, addressing challenges such as missing action labels and distribution shift. 
...
\textcite{McCarthy_2025} says, Video \acp{FM} can be highly applicable for the following \ac{LfV} use-cases, see~\autoref{fig:VideoFoundationModellingForLfV}:
\begin{itemize}
    \item \textbf{Finetune Pretrained Video \acp{FM}:} 
    ...
    \item \textbf{Leveraging Techniques and Datasets Inspired by Video \acp{FM}:}
    Techniques developed for video foundation models can be integrated into robot learning pipelines and can lead to better extraction of representations and knowledge. This in turn, can lead to improved generalization and task understanding. For instance, a Robot \ac{FM} can be directly trained by combining robot data with large-scale video inspired by video \acp{FM} techniques \cite{Team2024OctoAO, rt12022arxiv}.
\end{itemize}


\begin{figure}[!h]
	\centering
	\includegraphics[width=0.9\linewidth]{./chapter5/media/VideoFoundationModellingForLfV}
	\caption[Video Foundation Models for \acs{LfV}.]{Video Foundation Models for Learning from Videos. The top green box shows different types of video \acp{FM} and their robotic applications. The blue boxes illustrate two ways they support Learning from Videos. Image source: \cite{McCarthy_2025}.}
...
% \end{itemize}
The learning goal during training is to learn a video encoder, to predict $Y$ from $X$. Otherwise a video encoder $p(z|X)$ can learn an intermediate representation $z$ that enable the prediction or alignment of $Y$ from the input video $X$:  $X \xrightarrow{p(z|X)} z \rightarrow Y$ \cite{McCarthy_2025}
...
\begin{itemize}
    \item \textbf{Prediction-Based Objectives}: The target $Y$ may correspond to future video frames, masked regions, motion information or other supervision signals. For example, in video prediction tasks, the model predicts the next frame $x_{t+1}$ from the previous observations $x_{\leq t}$ \cite{Gao2022SimVPSY, Jang2024VisualRL}. 
    ...
    \item \textbf{Joint-Embedding Objectives}: Joint-embedding approaches \cite{Oord2018RepresentationLW, Xian2023TowardsGR, bardes2023v} first map both the video input $X$ and the known target $Y$ into a shared latent embedding space. A loss function is then applied to align semantically similar embeddings: 
    \begin{equation}
        \begin{aligned}
            z_X = f(X),
            \qquad
...
\textit{Auto-Encoding Approaches} have also become highly effective for self-supervised video representation learning. Auto-encoding approaches learn video representations by reconstructing input data from compressed latent representations. Common approaches include:
\begin{itemize}
...
\begin{equation}
    p(\tau \mid c)
\end{equation}
where $\tau$ is a video sequence and $c$ denotes conditioning information.
%  such as past observations, actions, robot actions, language instructions or goal images \cite{Yang2024VideoAT}.
During training, the model learns to predict future visual outcomes from past context, either in pixel space or in a learned latent space \cite{McCarthy_2025}. These models implicitly learn priors over physical dynamics, object interactions and human behavior, making them relevant for robotic learning and \ac{IL} \cite{McCarthy_2025}.
Video prediction models can be conditioned on past observations, alternative actions $\hat{a} \in \hat{A}$ \cite{Bruce2024GenieGI}, language instructions \cite{Yang2024VideoAT, BarTal2024LumiereAS, Kondratyuk2023VideoPoetAL} or goal images \cite{Hoppe2022DiffusionMF}, enabling structured control over generated futures \cite{McCarthy_2025, Yang2024VideoAT}.
...
\begin{itemize}
    ...
    % (e.g. reaching a goal state or matching expert demonstrations)
    thereby directly connecting video prediction with \ac{IL} and planning.
...
\textit{Autoregressive and Masked Transformer Models} model videos as a sequence of discrete tokens, typically obtained from a vector-quantized autoencoder \ac{VQ-AE} \cite{Yan2021VideoGPTVG, Yu2023LanguageMB, Ge2022LongVG, Kondratyuk2023VideoPoetAL}
...
% Video prediction models can be conditioned on alternative actions $\hat{a} \in \hat{A}$ \cite{Bruce2024GenieGI}, language instructions \cite{Yang2024VideoAT, BarTal2024LumiereAS, Kondratyuk2023VideoPoetAL} or goal images \cite{Hoppe2022DiffusionMF}, enabling structured control over generated futures \cite{McCarthy_2025, Yang2024VideoAT}.
% In robotics, such conditioning effectively turns video prediction models into controllable world models, bridging perception, planning and \ac{IL}. With such a world model, a robot can perform counterfactual reasoning over actions, simulate future outcomes of different control strategies and evaluate how well predicted trajectories align with expert demonstrations or task objectives \cite{Wang2026WorldMF}.
...
Video-to-text models can be used to generate natural language descriptions of video content, for example for video question answering and video summarization \cite{McCarthy_2025}.
...
\begin{itemize}
    ...
    By learning to generate natural language descriptions of video content, video-to-text models learn to extract high-level semantic representations that capture object properties, actions, interactions and scene context, as well as temporal dynamics. These representations are critical for understanding complex robotic tasks or environments and can be transferred to downstream robotic tasks, improving generalization and task understanding compared to video-only or image-to-text models \cite{McCarthy_2025}. 
    % The learned representations can capture both high-level semantic information and temporal dynamics, which are critical for understanding complex robotic tasks and environments \cite{McCarthy_2025}.

    \item \textbf{Grounded Reasoning and Planning:}
    % what is with VL-Models?
    \acp{LLM} have demonstrated strong capabilities as high-level planners in robotics, but their lack of direct grounding in physical perception limit their reliability in real-world environments \cite{Ahn2022DoAI}.
    Video-to-text models address this limitation by conditioning reasoning directly on rich visual-temporal inputs \cite{Wake2023GPT4VisionFR, Huang2026RoboStreamWS, Nguyen2019V2CNetAD,McCarthy_2025}. This enables improved grounding of symbolic reasoning in observed physical states, better modeling of object interactions and environment dynamics and more consistent closed-loop reasoning, where predictions are continuously informed by perceptual feedback.
    % \begin{itemize}
...
\begin{itemize}
    ...
    More recent approaches integrate video and text into a single autoregressive framework, instead of combining separate models \cite{team2023gemini, Liu2024WorldMO, Jin2023UnifiedLP}. In these native multimodal models, video inputs are tokenized into discrete visual tokens and processed jointly with text tokens as a single sequence.
    % More recent approaches integrate video and text into a unified single autoregressive framework, instead of combining separate models \cite{team2023gemini, Liu2024WorldMO, Jin2023UnifiedLP}.     
    ...
    The model can be trained for using next-token prediction:
    \begin{equation}
        p(x_t \mid x_{<t}),
    \end{equation}
    where tokens may correspond to either visual or textual information. As a result, these any-to-text or any-to-any models allow video, text and potentially other modalities to interact within the same latent space. This leads to a tighter coupling between vision and language representations and moves towards more general multi-modal foundation models capable of flexible cross-modal generation and reasoning \cite{McCarthy_2025}.
    % This formulation enables a unified treatment of different modalities within a single transformer architecture.
    % As a result, any-to-text or any-to-any models allow video, text and potentially other modalities to interact within the same latent space.
    % context window.
...
Multi-modal models jointly learn from multiple data modalities, which can include for example text, images, videos, and robot actions \cite{Reed2022AGA}. Unlike specialized vision-language models, these approaches aim to learn a unified sequence model that can process heterogeneous modalities within a shared architecture, that can connect visual perception, language understanding and robot control \cite{Yin2023ASO, Kawaharazuka2025VisionLanguageActionMF}.
% \paragraph{Methods.}
One possibility to train multi-modal models is using autoregressive next-token prediction objectives over interleaved sequences of tokens from different modalities \cite{Yin2023ASO}. 
% Depending on the model, tokens may represent text, image patches, video frames, proprioceptive robot observations or actions. 
...
Multi-modal models are in several ways relevant to robotics \cite{McCarthy_2025, Eze2024LearningBW}:
\begin{itemize}
    \item \textbf{High-Quality Representations:} By training together on robot data with web-scale video, image and text data, these models learn a shared latent space that bridges perception, language and control. This enables positive transfer and can serve as general-purpose representation backbones for robotic learning \cite{Kawaharazuka2025VisionLanguageActionMF}.
    
    \item \textbf{Knowledge Modality Initialization:} Multi-modal models can serve as strong initial \acp{KM}, such as Policies, Reward Models, Value Functions or Dynamics Models. For Example, \ac{RL} can fine-tune an initial policy  using task-specific rewards. This two-stage approach has been shown to improve sample efficiency and final performance compared to \ac{RL} from scratch \cite{Kawaharazuka2025VisionLanguageActionMF}.
...
Current models may \textbf{hallucinate} or fail to capture fine-grained spatial relationships, precise spatio-temporal dynamics and long-term temporal dependencies \cite{Lin2023VideoLLaVALU, Maaz2023VideoChatGPTTD, Yang2023LearningIR}. 
Another key bottleneck is the \textbf{limited quality and scale of low-level annotated video data}, particularly for tasks requiring detailed fine-grained understanding. 
Additionally, processing \textbf{high-dimensional and long video sequences} remains computationally expensive \cite{McCarthy_2025}, motivating for more efficient model architectures \cite{Yang2024VideoAT}. 
% \begin{itemize}
...
%     \item Additionally, processing \textbf{high-dimensional and long video sequences} remains computationally expensive \cite{McCarthy_2025}, motivating for more efficient model architectures \cite{Yang2024VideoAT}. 
% \end{itemize}
Further directions include \ac{RL}-based finetuning to reduce hallucinations \cite{Black2023TrainingDM}, the integration of 3D information \cite{Zhen20243DVLAA3}, like depth to improve physical realism or to enhance single-step conditioning for fine-grained video prediction tasks \cite{Bruce2024GenieGI}.
...
\begin{figure}[!ht]
	...
	\caption[Alternative Action Representations: Label action-free videos with $\hat{a}$]{To label action-free videos, an alternative action $\hat{a}$ can be derived from video data instead of the missing action $a$. Image source: \cite{McCarthy_2025}.}
...
\begin{enumerate}
    ...
    \item \textbf{Distribution Shift}: There exists a significant domain gap between human/internet demonstrationvideos and robot execution, which can severely degrade the generalization of models (pre-)trained on video data. This includes differences in visual appearance, embodiment, dynamics and behavior patterns.
\end{enumerate}
...
To overcome this limitation, recent approaches introduce an \textbf{Alternative Action Representation} $\hat{a} \in \hat{A}$, which can be used to relabel action-free videos to $(o_t, \hat{a}_t, o_{t+1})$ and enable video-to-robot transfer despite missing action labels $a$ and distribution shift, see~\autoref{fig:RecoveringActionFepresentationsFromVideo_1}.
...
% To address these challenges, recent approaches introduce an alternative action representations $\hat{a} \in \hat{A}$ can be derived from video data, which can be used to label action-free videos and enable video-to-robot transfer despite missing action labels and distribution shift, see~\autoref{fig:RecoveringActionFepresentationsFromVideo_1}. 

% \subsection{Introduction to Alternative Action Representations}
\subsection{Alternative Action Representations: Introduction}
An alternative action representation is defined as:
\begin{equation}
    \hat{a} \in \hat{A},
\end{equation}
where
...
\begin{enumerate}
    ...
    \item A \textbf{Decoder} $\pi(a_t \mid \hat{a}_t, o_t)$ or \textbf{Alignment Model $a_t = f_{align}(\hat{a}_t)$} maps these latent actions to the robot's real motor commands, see section~\ref{sec:AlternativeActionDecoder}.    
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Bridging the Domain Gap:} $\hat{A}$ bridges the gap between human video data and robot control by focusing on action semantics rather than platform-specific motor commands \cite{McCarthy_2025}. 
    
    \item \textbf{Learning from Weak Labels:} $\hat{A}$ enables learning from raw or weakly labeled videos, since inferring high-level actions from video is easier than recovering low-level motor commands \cite{McCarthy_2025}.
    
    \item \textbf{Transferable Representations Across Embodiments:} $\hat{A}$ supports transfer across embodiments, because the same semantic action, e.g. ``open drawer'', can be instantiated by different robots with different kinematics \cite{McCarthy_2025}.
    ...
    \item \textbf{Structured Learning:} A well-structured representation $\hat{A}$ that is minimal, disentangled and semantically consistent across the state space encodes meaningful action information. This makes it easier to learn models that are predictive of both future observations (learning $p_{\text{alt}}(o_{t+1} \mid \hat{a}_t, o_t)$ instead of $p(o_{t+1} \mid o_t)$) and robot actions (learning $\pi(a_t \mid \hat{a}_t, o_t)$ instead of $\pi(a_t \mid o_t)$), leading to more data-efficient and generalizable policies, dynamics models, and value functions \cite{McCarthy_2025, Rybkin2018LearningWY, Bruce2024GenieGI}.
...
Once $\hat{A}$ is defined, the same representation can be used for example for policies $\pi_{\hat{A}}(\hat{a}_t \mid o_t)$ \cite{Schmidt2023LearningTA}, dynamics models $p_{\hat{A}}(o_{t+1} \mid o_t, \hat{a}_t)$ \cite{Bruce2024GenieGI} or state-action value functions $Q_{\hat{A}}(o_t, \hat{a}_t)$ \cite{Bhateja2023RoboticOR}, which further encourages consistent and transferable action abstractions.
% \begin{itemize}
...
\subsection{Alternative Action Representations: Categories}
...
\begin{figure}[h]
	...
	\caption[Alternative Action Representations: Categories]{Categories of Alternative Action Representations that can be learned or obtained from video data. Image source: \cite{McCarthy_2025}.}
...
These approaches learn latent actions in vectorized forms (=embeddings) that describe the transition from one observation to the next \cite{McCarthy_2025}:
\begin{equation}
...
For example skill embeddings can capture structures such as ``reach the object'', ``move the arm to the target location'' or ``grasp and lift'', which can later be decoded into a full action sequence.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Key-Frame Selection:} Identifies critical states in a trajectory that can serve as action representations \cite{Pertsch2019KeyframingTF, liu2023learning}, see~\autoref{tab:keyframes_pickplace}.
...
% \textit{Methods for Missing Language Captions:}
